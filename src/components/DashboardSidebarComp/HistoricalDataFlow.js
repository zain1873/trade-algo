...
import "../DashboardSidebarComp/styles/historicalDataFlow.css";
        ...
        const HistoricalDataFlow = ({ darkMode }) => {
          ...
          const [mediumCapsData, setMediumCapsData] = useState([]);
          const [smallCapsData, setSmallCapsData] = useState([]);
        
          // Fetch data for large, medium, and small caps
          useEffect(() => {
            async function loadData() {
              try {
                // Fetch Large Caps data
                const largeCapsResponse = await fetch('https://valourwealthdjango-production.up.railway.app/api/large_caps/');
                const largeCapsResult = await largeCapsResponse.json();
                setLargeCapsData(largeCapsResult.chunked_data);
        
                // Fetch Medium Caps data
                const mediumCapsResponse = await fetch('https://valourwealthdjango-production.up.railway.app/api/medium_caps/');
                const mediumCapsResult = await mediumCapsResponse.json();
                setMediumCapsData(mediumCapsResult.chunked_data);
        
                // Fetch Small Caps data
                const smallCapsResponse = await fetch('https://valourwealthdjango-production.up.railway.app/api/small_caps/');
                const smallCapsResult = await smallCapsResponse.json();
                setSmallCapsData(smallCapsResult.chunked_data);
              } catch (error) {
          ...
          const renderRows = (data) => {
            return data.map((chunk, index) => {
              const [ticker, fromPrice, fromTime, toPrice, toTime, irrVol, percChange, duration] = chunk;
              return (
                <tr key={index}>
                  <td>{ticker}</td>
                  <td>
                  ...
                  <td>
                    <span className="badge bg-success">{irrVol}</span>
                  </td>
                  <td>
                    {percChange}
                    <br />
            ...
            >
              ...
              <div className="table-responsive mt-3">
                <table className="table table-bordered table_history">
                  ...
                  >
                    {/* Render rows based on the active tab */}
                    {activeTab === "largeCaps" && renderRows(largeCapsData)}
                    {activeTab === "mediumCaps" && renderRows(mediumCapsData)}
                    {activeTab === "smallCaps" && renderRows(smallCapsData)}
                  </tbody>
                </table>
              </div>
            </div>
