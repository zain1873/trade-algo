      ...
      <div className="mb-4">
        ...
        <div className="d-flex justify-content-between align-items-center mb-3">
          <ul className="nav nav-tabs flex-grow-1 intrady-table-tabs">
            <li className="nav-item">
              <button 
                className={`nav-link ${activeTab === 'intraday' ? 'active' : ''}`}
        ...
        <div className="cap-tabs mb-3">
          <ul className="nav nav-pills cap-nav">
            <li className="nav-item">
              <button 
                className={`cap-btn ${activeCapSize === 'auto' ? 'active' : ''}`}
            ...
            <li className="nav-item">
              <button 
                className={`cap-btn ${activeCapSize === 'large' ? 'active' : ''}`}
            ...
            <li className="nav-item">
              <button 
                className={`cap-btn ${activeCapSize === 'medium' ? 'active' : ''}`}
            ...
            <li className="nav-item">
              <button 
                className={`cap-btn ${activeCapSize === 'small' ? 'active' : ''}`}
        ...
        <div className="alert alert-info p-2 small">
          <strong>Currently viewing:</strong> {activeTab === 'intraday' ? 'Intraday Activity' : activeTab === 'weekly' ? 'Weekly Activity' : 'Popular Stocks'} 
          {activeCapSize !== 'auto' && ` - ${activeCapSize === 'large' ? 'Large Caps' : activeCapSize === 'medium' ? 'Medium Caps' : 'Small Caps'}`}
        </div>
      </div>
          ...
          <tbody>
            {stockData.map((stock, index) => (
              <tr key={index}>
                ...
                <td>
                  <span className={`sentiment-pill ${getNewsSentimentClass(stock.newsSentiment)}`}>
                    {stock.newsSentiment > 0 ? `+${stock.newsSentiment}` : stock.newsSentiment}
                  </span>
                </td>
                <td>
                  <span className={`ai-score-pill badge ${getAIScoreClass(stock.aiScore)}`}>
                    {stock.aiScore}
                  </span>
                </td>
                <td className="text-success fw-medium">{stock.darkPoolActivity}</td>
